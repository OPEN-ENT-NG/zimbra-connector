...
export let zimbraController = ng.controller("ZimbraController", [
    ...
    function($location, $scope, $timeout, $compile, $sanitize, model, route) {
        ...
        $scope.sendMail = async () => {
            ...
            $timeout(() => $scope.sending = false, $scope.zimbra.configSendTimeout);
        ...
        $scope.isRecallable = function(mail: Mail): boolean {
            return mail.returned === 'NONE' &&
                ($scope.containsInternal(<any>mail.to) || $scope.containsInternal(<any>mail.cc) || $scope.containsInternal(<any>mail.bcc)) &&
